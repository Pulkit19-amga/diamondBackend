...
//Route::get('/diamonds/by-shape', [DiamondMasterController::class, 'filterByShape']);
/**
 * Summary of ShapeController
 * const fetchDiamondsByShape = async (shapeId) => {
  try {
    const response = await axios.get('/api/diamonds/by-shape', {
      params: { shape_id: shapeId }
    });

    if (response.data.success) {
      setDiamonds(response.data.data); // update your React state
    }
  } catch (error) {
