...
<div class="container py-4">
  <h4>Edit Diamond</h4>
  <form action="{{ route('diamond-master.update', $diamond->diamondid) }}" method="POST">
    @csrf @method('PUT')
    <div class="row g-3">
      @php
        $fields = [
        ...
        <div class="col-md-4">
          ...
          @if($type==='select')
            <select name="{{ $name }}" id="{{ $name }}" class="form-select @error($name) is-invalid @enderror">
              ...
              @foreach($opts as $k=>$v)
                <option value="{{ $k }}" {{ old($name, $diamond->$name)=="$k"?'selected':'' }}>{{ $v }}</option>
              @endforeach
            </select>
          @else
            <input type="{{ $type }}" name="{{ $name }}" id="{{ $name }}" class="form-control @error($name) is-invalid @enderror" value="{{ old($name, $diamond->$name) }}">
          @endif
          @error($name)
